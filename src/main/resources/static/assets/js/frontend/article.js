...
var ani =
    ...
    '                   <span class="layui-btn layui-btn-radius layui-btn-sm" style="background: #F44336;"> {{ item.cate.cnName }}</span>' +
    '                   {{# if(item.top){ }}' +
    ...
    '<span class="dtime">{{ BMY.dateFormatter(item.post) }}</span>' +
    '                   </div>' +
    '               </div>' +
    '           </fieldset>' +
    '       </div>' +
...
//     var s = $(span).text();
//     s = s.substring(1);
//     location.href = "/pages?tagSearch=" + s;
// }